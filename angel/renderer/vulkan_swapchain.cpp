...
bool VulkanSwapchain::create_swapchain(const u32 framebuffer_width, const u32 framebuffer_height) {
    if (!_initialized) {
        AL_CORE_ERROR("(Vulkan-Swapchain) Swapchain has not been initialized. Initialize the swapchain first.")
        return false;
    }
    ...
    ANGEL_ASSERT(surface_formats.size() > 0);
    ...
    ANGEL_ASSERT(swapchain_image_count == image_count)
