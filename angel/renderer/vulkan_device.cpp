...
void VulkanDevice::setup_device(VkInstance const &instance,
    ...
    AL_CORE_INFO(device_info_str.c_str());
}

bool VulkanDevice::create_logical_device() {
    if (!setup) {
        AL_CORE_ERROR("(Vulkan-Device) Must setup device first")
        return false;
    }
    AL_CORE_INFO("(Vulkan-Device) Creating logical device...")
    AL_CORE_INFO("(Vulkan-Device) Creating device queues...")
    ...
    for (u32 const &index : queue_family_indices) {
        ...
        ANGEL_ASSERT(queue_priorities.size() >= actual_queue_creation_count);
        ...
        AL_CORE_DEBUG("(Vulkan-Device) Creating %d queues for queue family %d", actual_queue_creation_count, index);
    }
    ...
    if (_device_requirements.dynamic_rendering) {
        ANGEL_ASSERT(_device_properties.device_features_13.dynamicRendering == VK_TRUE);
        device_features_13.dynamicRendering = VK_TRUE;
        AL_CORE_DEBUG("(Vulkan-Device) Enabling dynamic rendering");
    }
    ...
    AL_CORE_INFO("(Vulkan-Device) Successfully created logical device.")
...
bool vulkan_get_physical_device_properties(VkPhysicalDevice const &device,
    ...
    for (u64 i = 0; i < out_properties.device_queues.size(); ++i) {
        ...
        ANGEL_ASSERT(i != QUEUE_INDEX_NONE)
...
bool vulkan_physical_device_meets_requirements(VulkanPhysicalDeviceProperties const &properties,
    ...
    if (requirements.allow_integrated && device_type == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU)
        device_type_requirement_met = true;
    if (device_type_requirement_met) AL_CORE_DEBUG("(Vulkan-Device) Device meets device type requirements.")
    ...
    if (requirements.dynamic_rendering && vkb_to_b(!properties.device_features_13.dynamicRendering)) {
        AL_CORE_DEBUG("(Vulkan-Device) Dynamic rendering is required but is not found on device.")
        device_meets_feature_requirements = false;
    }
    if (device_meets_feature_requirements) AL_CORE_DEBUG("(Vulkan-Device) Device meets feature requirements.")
    ...
    if (device_meets_queue_requirements) AL_CORE_DEBUG("(Vulkan-Device) Device meets queue family requirements.")

    bool has_swapchain_support = properties.surface_present_modes.size() > 0 && properties.surface_formats.size() > 0;
    if (has_swapchain_support) AL_CORE_DEBUG("(Vulkan-Device) Device meets swapchain requirements.")

    bool has_required_extensions = true;
    for (const char *const &require : requirements.required_extensions) {
        AL_CORE_DEBUG("(Vulkan-Device) Checking if device has required extension %s", require)
        bool found = false;
        for (VkExtensionProperties const &extension : properties.extension_properties) {
            if (std::strcmp(extension.extensionName, require) == 0) {
                found = true;
                AL_CORE_DEBUG("(Vulkan-Device) Extension %s found", require)
            }
        }
        if (!found) {
