...
bool vulkan_create_instance(VulkanCreateInstanceInfo const &create_info, VkInstance *out_instance) {
    AL_CORE_INFO("(Vulkan Instance) Creating vulkan instance...");

    // Check API Version
    AL_CORE_INFO("(Vulkan Instance) Checking if API version of (major.minor.patch) %d.%d.%d is met",
    ...
    AL_CORE_INFO("(Vulkan Instance) Found API version of (major.minor.patch) %d.%d.%d",
                 api_major,
                 api_minor,
                 api_patch);
    if (!api_version_requirements_met) {
        AL_CORE_FATAL("(Vulkan Instance) API version is not met.");
        return false;
    }
...
#if defined(ANGEL_PLATFORM_WINDOWS)
        "VK_KHR_win32_surface",
#elif defined(ANGEL_PLATFORM_LINUX)
    ...
    if (check_required_extensions(extension_names) && check_required_layers(layer_names)) {
        AL_CORE_INFO("(Vulkan Instance) All required extensions and layers found.")
    } else {
        AL_CORE_ERROR("(Vulkan Instance) Failed to find required extensions and layers")
        return false;
    }

    void *vk_instance_p_next = nullptr;
    if (create_info.enable_debug_utils_extension) {
        AL_CORE_INFO("(Vulkan Instance) Creating debug utils extension as extension to vulkan instance");
    ...
    AL_CORE_INFO("(Vulkan-Instance) Successfully created vulkan instance")
    return true;
}
...
static VkBool32 debug_utils_messenger_callback(VkDebugUtilsMessageSeverityFlagBitsEXT message_severity,
    ...
    switch (message_severity) {
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
            AL_CORE_ERROR("(Vulkan-DEBUG_CALLBACK):\n%s", message);
            break;
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
            AL_CORE_WARN("(Vulkan-DEBUG_CALLBACK):\n%s", message);
            break;
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
            AL_CORE_INFO("(Vulkan-DEBUG_CALLBACK):\n%s", message);
            break;
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
            AL_CORE_TRACE("(Vulkan-DEBUG_CALLBACK):\n%s", message);
            break;
        default:
            break;
    }
...
static bool check_required_extensions(std::vector<const char *> const &extension_names) {
    AL_CORE_INFO("(Vulkan Instance) Enumerating and checking for required extensions (%d)...", extension_names.size());
    ...
    AL_CORE_TRACE("(Vulkan Instance) Enumerating extensions (%d)...", available_extension_count)
    for (VkExtensionProperties const &extension : available_extensions) {
        AL_CORE_TRACE("(Vulkan Instance) Found extension: %s", extension.extensionName)
    }

    for (const char *const &extension_name : extension_names) {
        AL_CORE_DEBUG("(Vulkan Instance) Searching for extension %s", extension_name);
        bool found = false;
        ...
        if (!found) {
            AL_CORE_ERROR("(Vulkan Instance) Could not find extension.");
            return false;
        } else {
            AL_CORE_DEBUG("Extension found.")
        }
    }
...
static bool check_required_layers(std::vector<const char *> const &layer_names) {
    AL_CORE_INFO("(Vulkan Instance) Enumerating and checking for required layers (%d)...", layer_names.size());
    ...
    for (const char *const &layer_name : layer_names) {
        AL_CORE_DEBUG("(Vulkan Instance) Searching for layer %s", layer_name);
        bool found = false;
        ...
        if (!found) {
            AL_CORE_ERROR("(Vulkan Instance) Could not find layer.");
            return false;
        } else {
            AL_CORE_DEBUG("Layer found.")
        }
    }
    return true;
}
