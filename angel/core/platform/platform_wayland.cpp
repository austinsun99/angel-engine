...
const static struct {
    static void keymap(void *data, struct wl_keyboard *wl_keyboard, uint32_t format, int32_t fd, uint32_t size) {
        ...
        ANGEL_ASSERT(format == WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1)

        char *map_shm = static_cast<char *>(mmap(nullptr, size, PROT_READ, MAP_PRIVATE, fd, 0));

        ANGEL_ASSERT(map_shm != MAP_FAILED)
        ANGEL_ASSERT(internal->xkb_context != nullptr)
    ...
    static void key(void *data,
        ...
        ANGEL_ASSERT(sym != XKB_KEY_NoSymbol)
...
bool Window::open_window(const WindowConfig config) {
    ...
    if (internal->xkb_context == nullptr) {
        AL_CORE_FATAL("Wayland: Could not obtain xkb context")
        return false;
    }

    internal->wl_display = wl_display_connect(nullptr);
    if (internal->wl_display == nullptr) {
        AL_CORE_FATAL("Wayland: Could not connect to display")
        return false;
    }
