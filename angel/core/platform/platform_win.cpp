...
bool Window::open_window(const WindowConfig config) {
    ...
    AL_CORE_INFO("Starting open window process...");
    if (GetModuleHandleEx(0, nullptr, &internal->instance) == 0) {
        log_win_error_msg_fatal(GetLastError());
        return false;
    }
    AL_CORE_INFO("Windows (1/3): Obtained module handle")
    ...
    internal->hwnd = CreateWindowEx(ex_wnd_style,
                                    ...
                                    internal->instance,
                                    this);
    AL_CORE_INFO("Windows (2/3): Window has been created")
    ...
    AL_CORE_INFO("Windows (3/3): Showing window...")
...
void Window::on_window_close() {
    AL_CORE_INFO("Windows: closing window")
}
...
static LRESULT window_callback(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
    ::angel::platform::Window *state;
    if (msg == WM_CREATE) {
        CREATESTRUCT *create = reinterpret_cast<CREATESTRUCT *>(lparam);
        state                = reinterpret_cast<::angel::platform::Window *>(create->lpCreateParams);
        SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(state));
        return TRUE;
    }

    LONG_PTR data = GetWindowLongPtr(hwnd, GWLP_USERDATA);
    state         = reinterpret_cast<::angel::platform::Window *>(data);
    ...
    switch (msg) {
        ...
        case WM_SYSKEYUP: {
            ...
            const ::angel::input::Keycode key = static_cast<::angel::input::Keycode>(wparam);
        ...
        case WM_RBUTTONUP: {
            ...
            ::angel::input::MouseButton mouse_button = ::angel::input::MouseButton::MAX_BUTTONS;
            switch (msg) {
                case WM_LBUTTONDOWN:
                case WM_LBUTTONUP:
                    mouse_button = ::angel::input::MouseButton::MOUSE_LEFT;
                    break;
                case WM_MBUTTONDOWN:
                case WM_MBUTTONUP:
                    mouse_button = ::angel::input::MouseButton::MOUSE_MIDDLE;
                    break;
                case WM_RBUTTONDOWN:
                case WM_RBUTTONUP:
                    mouse_button = ::angel::input::MouseButton::MOUSE_RIGHT;
                    break;
            }
